...
class DeleteReview(View):
    def delete(self, request, video_id, review_id, *args, **kwargs):
        try:
           ...
           # 성공 메시지 반환
            return JsonResponse({}, status=204)  # 응답 본문 없이 상태 코드 204를 반환
...
class PutReview(View):
    def put(self, request, video_id, review_id, *args, **kwargs):
        try:
            ...
            if not nickname or not new_rating or not new_comments:
                return JsonResponse({'error': 'nickname, new_rating, and new_comments parameters are required'}, status=400)
            ...
            # 리뷰가 실제로 존재하는지 확인하고, 리뷰 ID와 동영상 ID, 사용자 ID가 일치하는지 확인
            cursor.execute("""
                SELECT * FROM review
                WHERE review_id = %s AND video_id = %s AND users_id = %s;
            """, (review_id, video_id, user_id))
            ...
            query = """
                ...
                WHERE review_id = %s AND video_id = %s AND users_id = %s;
            """
            cursor.execute(query, (new_rating, new_comments, review_id, video_id, user_id))
            conn.commit()
